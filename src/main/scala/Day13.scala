  ...
  def main(args: Array[String]) {
    val input =
      """Alice would lose 2 happiness units by sitting next to Bob.
        ...
        |Mallory would lose 99 happiness units by sitting next to George.""".stripMargin.split("\n")
    val pattern = "([A-z]+) would ([a-z]+) ([0-9]+) happiness units by sitting next to ([A-z]+)\\.".r

    val rules: Array[Rule] = input.map {
      case pattern(who, what, howmany, whom) => Rule(who, howmany.toInt * (what match {
        case "gain" => 1
        case _ => -1
      }), whom)
    }

    def neighbors[T](i: Int, in: Seq[T]): List[T] = i match {
      case 0 => in(i + 1) :: in.last :: Nil
      case _ if i == in.size - 1 => in(i - 1) :: in.head :: Nil
      case _ => in(i - 1) :: in(i + 1) :: Nil
    }


    def getOptimal(rules: Array[Rule]) = {
      val people = rules.map(_.who).distinct

      val peopleRuleMap: Map[String, Map[String, Int]] = rules.map(_.who).distinct.map { s =>
        s -> rules.filter(_.who == s).map { rule =>
          rule.whom -> rule.change
        }.toMap
      }.toMap

      val combs = people.permutations.toList
      println(s"${combs.size} combinations")
      combs.map { comb =>
        comb.indices.map { i =>
          val it = comb(i)
          val sibs = neighbors(i, comb)
          sibs.map(peopleRuleMap(it)).sum
        }
      }.map(_.sum).max
    }
    println(getOptimal(rules))

    val otherPeople = rules.map(_.who).distinct
    val me = "Me"
    val rulesForMe = otherPeople.flatMap(human => List(Rule(human, 0, me), Rule(me, 0, human)))
    println(getOptimal(rulesForMe ++ rules))

  }
}

case class Rule(who: String, change: Int, whom: String)
