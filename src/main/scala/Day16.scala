...
object Day16 {
  val input =
    """Sue 1: cars: 9, akitas: 3, goldfish: 0
      ...
      |Sue 500: pomeranians: 10, cats: 3, vizslas: 5""".stripMargin.split("\n")

  val sues = input.map { s =>
    val split = s.split(": ")
    val idInt = split.head.split("Sue ").last.toInt
    val id = Some(idInt)
    val stuff = split.tail.mkString(": ").split(", ")
    def getFromStuff(key: String) = stuff.find(_ contains key).map(_.split(": ").last.toInt)
    val mappedKeys = keys.map(key => key -> getFromStuff(key)).collect { case (key, Some(value)) => key -> value }.toMap
    new Sue(id, mappedKeys)
  }
  ...
  def main(args: Array[String]) {
    val master =
      """children: 3
        ...
        |trees: 3
        |cars: 2
        |perfumes: 1""".stripMargin.split("\n").map(_.split(": ")).map { split =>
        split.head -> split.last.toInt
      }.toMap
    val masterSue = Sue(None, master)
    println(sues.map(another => (another.id, masterSue.matches(another))).sortBy(s => -s._2).head)
  }
}

case class Sue(id: Option[Int], info: Map[String, Int]) {
  val listOfContents = keys.map(key => key -> info.get(key))
  val veryLow = -99999

  def matches(another: Sue) = (listOfContents zip another.listOfContents).map {
    case ((key, Some(a)), (_, Some(b))) => key match {
      case "cats" if a < b => 1
      case "trees" if a < b => 1
      case "pomeranians" if a > b => 1
      case "goldfish" if a > b => 1
      case _ if a == b => 1
      case _ => veryLow
    }
    case _ => 0
...
object Sue {
  val keys = List("children", "cats", "samoyeds", "pomeranians", "akitas", "vizslas", "goldfish", "trees", "cars", "perfumes")
}
