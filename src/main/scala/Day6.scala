...
object Day6 {
  ...
  def sol1() {
    val input =
      """turn on 887,9 through 959,629
        ...
        |turn off 102,229 through 674,529""".stripMargin.split("\n")

    val lights: mutable.Map[(Int, Int), Boolean] = mutable.Map((0 to 999).flatMap { x => (0 to 999).map { y => (x, y) } }.map { b => b -> false }: _*)

    def light(x: Int, y: Int) = lights((x, y))

    def turnOn(x: Int, y: Int) = lights((x, y)) = true
    def turnOff(x: Int, y: Int) = lights((x, y)) = false
    def toggle(x: Int, y: Int) = lights((x, y)) = !lights((x, y))

    def updateLights(x1: String, y1: String, x2: String, y2: String)(command: (Int, Int) => Unit) =
      (x1.toInt to x2.toInt).foreach { x => (y1.toInt to y2.toInt).foreach { y => command(x, y) } }

    val through = "([0-9]+),([0-9]+) through ([0-9]+),([0-9]+)"
    val turnOnR = s"turn on $through".r
    val turnOffR = s"turn off $through".r
    val toggleR = s"toggle $through".r


    input.foreach {
      case turnOnR(x1, y1, x2, y2) => updateLights(x1, y1, x2, y2)(turnOn)
      case turnOffR(x1, y1, x2, y2) => updateLights(x1, y1, x2, y2)(turnOff)
      case toggleR(x1, y1, x2, y2) => updateLights(x1, y1, x2, y2)(toggle)
    }

    println(lights.values.count(_ == true))
  }
  ...
  def sol2() {
    ...
    val lights: mutable.Map[(Int, Int), Int] = mutable.Map((0 to 999).flatMap { x => (0 to 999).map { y => (x, y) } }.map { b => b -> 0 }: _*)

    def turnOn(x: Int, y: Int) = lights((x, y)) += 1
    def turnOff(x: Int, y: Int) = lights((x, y)) = 0 max (lights((x, y)) - 1)
    def toggle(x: Int, y: Int) = lights((x, y)) += 2
