object Day8 {
  val input =
    """"\xa8br\x8bjr\""
      ...
      |"qludrkkvljljd\\xvdeum\x4e"""".stripMargin.split("\n")
  ...
  def sol1(){
    def removeQuotesOutside(x: String) = x.drop(1).dropRight(1)
    def removeQuotesInside(x: String) = x.replaceAll("\\\\\"", "_")
    def removeBackslashes(x: String) = x.replace("\\\\", "_")
    def removeHex(x: String) = x.replaceAll("\\\\(x)([0-9A-Fa-f]{2})", "_")
    def transform(x: String) = removeBackslashes(removeQuotesOutside(removeQuotesInside(removeHex(x))))
    println(input.map(_.length).sum - input.map(transform).map(_.length).sum)
  }
  def sol2(){
    def addQuotesInside(x: String) = x.replace(""""""", "||")
    def addBackslashes(x: String) = x.replace("""\""", """||""")
    def addToXs(x: String) = x.replaceAll("\\\\(x)", """||x""")
    def transform(x: String) = "\"\\\"" + addToXs(addBackslashes(addQuotesInside(x.drop(1).dropRight(1)))) + "\"\\\""

    println(input.map(transform).map(_.length).sum - input.map(_.length).sum)
  }
}
