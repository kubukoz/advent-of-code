...
import com.kubukoz.adventofcode2016.Material._
...
class Day11Tests extends FlatSpec with Matchers {
  ...
  private val parsedStartState = State(0, Map(
    0 -> Floor(Set(hydrogen, lithium), Set()),
    1 -> Floor(Set(), Set(hydrogen)),
    2 -> Floor(Set(), Set(lithium)),
    3 -> Floor(Set(), Set())
  ), 0)
  ...
  private val endgame = State(
    3, Map(
      ...
      3 -> Floor(Set(hydrogen, lithium), Set(hydrogen, lithium))
    ), 0
  )
  ...
  it should "be false for the second correct step" in {
    State(1, Map(
      0 -> Floor(Set(lithium), Set()),
      1 -> Floor(Set(hydrogen), Set(hydrogen)),
      2 -> Floor(Set(), Set(lithium)),
      3 -> Floor(Set(), Set())
  ...
  it should "be false for the third correct step" in {
    State(2, Map(
      0 -> Floor(Set(lithium), Set()),
      1 -> Floor(Set(), Set()),
      2 -> Floor(Set(hydrogen), Set(lithium, hydrogen)),
      3 -> Floor(Set(), Set())
    ), 0).blowsUp shouldBe false
  }
  ...
  it should "be false for the fourth correct step" in {
    State(1, Map(
      0 -> Floor(Set(lithium), Set()),
      1 -> Floor(Set(hydrogen), Set()),
      2 -> Floor(Set(), Set(lithium, hydrogen)),
      3 -> Floor(Set(), Set())
    ), 0).blowsUp shouldBe false
  }
